...
export class ThemeManager {
  ...
  toggleMode(): void {
    ...
    if (currentMode === 'system') newMode = this.systemTheme === 'dark' ? 'light' : 'dark';
  ...
  private notifyListeners(): void {
    ...
    this.listeners.forEach(listener => {
      try { listener(themeState); } catch (error) { logger.error('Theme listener error', error as Error); }
    });
  }
  ...
  getCSSCustomProperties(): Record<string, string> {
    ...
    if (this.preferences.highContrast) properties['--contrast-multiplier'] = '1.5';
    if (this.preferences.compactMode) properties['--spacing-scale'] = '0.85';
    return properties;
  }
...
export function ThemeProvider({ children }: ThemeProviderProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = themeManager.subscribe((theme) => { setThemeState(theme); });
    return unsubscribe;
  }, []);
...
export function useTheme() {
  ...
  const updatePreferences = (updates: Partial<ThemePreferences>) => { if (themeManager) themeManager.updatePreferences(updates); };
  const toggleMode = () => { if (themeManager) themeManager.toggleMode(); };
  const resetToDefaults = () => { if (themeManager) themeManager.resetToDefaults(); };

  return { ...context, updatePreferences, toggleMode, resetToDefaults, isDarkMode: context.resolvedTheme === 'dark', isLightMode: context.resolvedTheme === 'light', isSystemMode: context.mode === 'system' };
}
...
export const ThemeUtils = {
  ...
  createMediaQueries(): Record<string, string> {
    return { light: '(prefers-color-scheme: light)', dark: '(prefers-color-scheme: dark)', reducedMotion: '(prefers-reduced-motion: reduce)', highContrast: '(prefers-contrast: high)' };
  }
};

export const COLOR_SCHEMES = {
  blue: { primary: 'rgb(59, 130, 246)', primaryDark: 'rgb(37, 99, 235)', secondary: 'rgb(147, 197, 253)', accent: 'rgb(219, 234, 254)' },
  green: { primary: 'rgb(34, 197, 94)', primaryDark: 'rgb(22, 163, 74)', secondary: 'rgb(134, 239, 172)', accent: 'rgb(220, 252, 231)' },
  purple: { primary: 'rgb(147, 51, 234)', primaryDark: 'rgb(126, 34, 206)', secondary: 'rgb(196, 181, 253)', accent: 'rgb(237, 233, 254)' },
  orange: { primary: 'rgb(249, 115, 22)', primaryDark: 'rgb(234, 88, 12)', secondary: 'rgb(253, 186, 116)', accent: 'rgb(255, 237, 213)' },
  red: { primary: 'rgb(239, 68, 68)', primaryDark: 'rgb(220, 38, 38)', secondary: 'rgb(252, 165, 165)', accent: 'rgb(254, 226, 226)' },
} as const;
